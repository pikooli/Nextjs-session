...
const NS = ['navbar', 'common'];

const getTranslations = (mainGetServerSideProps?: GetServerSideProps | string[], translationNs?: string[] ) => {
  return async (props: GetServerSidePropsContext) => {
    const { locale = 'fr' } = props;
    const mainProps = mainGetServerSideProps && typeof(mainGetServerSideProps) === 'function' ? await mainGetServerSideProps(props) : { props: {} };
    translationNs = typeof(mainGetServerSideProps) !== 'function' ? mainGetServerSideProps : translationNs;
    return _.merge(mainProps,
      { props: {...await serverSideTranslations(locale, [ ...NS, ...translationNs ? translationNs : []])}}
    )
  }
}
